...
#用sqlalchemy创建引擎
sql = "select * from city_wcdma_day where 地市 <>'其他'  AND 日期>= '" + tdate['startDate'] + "' AND 日期 < '" + tdate['endDate'] + "'"
...
writer = pd.ExcelWriter(filePath + tdate['startDate'] + '_' + tdate['endDate'] + '_WCDMA.xlsx')       #保存表格为excel      文件名称为本月起始日期_结束日期_WCDMA.xlsx
